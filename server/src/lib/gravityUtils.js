...
export function calculateGravityScore(feature, maxVotes) {
  ...
  const votesNorm = maxVotes > 0 ? (votes / maxVotes) * 5 : 0;
...
export function recalculateAllGravityScores(db) {
  try {
    // 1. Get max votes
    const { maxVotes } = db.prepare('SELECT MAX(vote_count) as maxVotes FROM features').get();

    // 2. Get all features with relevant scoring fields
    const features = db.prepare('SELECT id, vote_count, impact, effort, priority FROM features').all();
