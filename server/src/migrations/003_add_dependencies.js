...
/**
 * One-time migration: adds an empty dependencies array to all existing
 * feature documents that don't already have one.
 *
 * Usage:
 *   node server/src/migrations/003_add_dependencies.js
 */
...
async function run() {
  try {
        ...
        batch.map(async (feature) => {
          try {
            await featuresContainer.item(feature.id, feature.id).patch([
              { op: 'add', path: '/dependencies', value: [] },
            ]);
            updatedCount++;
          } catch (err) {
            if (err.code === 409) {
              console.log(`Skipping ${feature.id}: dependencies already exists`);
            } else {
    ...
    console.log(`Migrated ${updatedCount} features: added dependencies field.`);
  } catch (error) {
